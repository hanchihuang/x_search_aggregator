...
SUCCESS_RE = re.compile(r"成功收集\s+(\d+)\s+条(?:推文|评论)(?:（目标:\s*(\d+)条）)?")
EMAIL_RE = re.compile(r"^[^@\s]+@[^@\s]+\.[^@\s]+$")
RUN_DIR_RE = re.compile(r"(?:Run directory|运行目录|输出目录):\s*(.+)")
...
def run_post_comments_job(
    ...
    if auto_launch:
        cmd.append("--auto-launch")
    code = run_command_stream(task_id, cmd, "正在抓取指定帖子评论", 5)
    if code != 0:
        raise RuntimeError("指定帖子评论抓取失败，请检查日志。")

    run_dir = detect_newest_dir(before)
    if run_dir is None:
        raise RuntimeError("帖子评论抓取完成，但未找到输出目录。")

    update_task(
        task_id,
        result_dir=str(run_dir),
        message=f"指定帖子与评论抓取完成：{post_url}",
        stage="已完成",
        progress=100,
    )
...
def run_zhihu_user_job(task_id: str, user_url: str, cookie: str, user_agent: str, headless: bool) -> None:
    ...
    if code != 0:
        raise RuntimeError(explain_command_failure(task_id, "知乎用户动态抓取失败，请检查日志。"))
...
def worker(task_id: str) -> None:
    ...
    try:
        ...
        elif task["type"] == "post_comments":
            run_post_comments_job(
                task_id,
                params["post_url"],
                params["state"],
                params["headless"],
                int(params.get("max_comments", 0)),
                int(params.get("max_scrolls", 400)),
                int(params.get("no_new_stop", 12)),
                int(params.get("scroll_pause", 1800)),
                params.get("cdp_url", ""),
                params.get("auto_launch", False),
            )
  ...
  <main class="wrap">
    ...
    <section class="layout">
      <div class="stack">
        ...
        <form class="panel js-task-form" data-kind="post_comments">
          <h2>抓取指定 X 帖子及全部评论</h2>
          <p>输入具体帖子链接后，系统会打开详情页，持续向下滚动展开并抓取评论，周期性保存 `post.json`、`comments.json`、`comments.csv`、`summary.md` 和 `article.html`。</p>
          <label>帖子链接
            <input type="text" name="post_url" placeholder="https://x.com/karpathy/status/2036836816654147718" required />
          </label>
          <label>登录态文件
            <input type="text" name="state" value="auth_state_cookie.json" />
          </label>
          ...
          <label>评论上限
            <input type="text" name="max_comments" value="0" />
          </label>
          <label>最大滚动轮数
            <input type="text" name="max_scrolls" value="400" />
          </label>
          <label>连续无新增停止轮数
            <input type="text" name="no_new_stop" value="12" />
          </label>
          <label>滚动间隔（毫秒）
            <input type="text" name="scroll_pause" value="1800" />
          </label>
          <label class="checkbox">
            <input type="checkbox" name="headless" value="1" checked />
            <span>无头模式运行</span>
          ...
          <div class="mini-note">`评论上限` 填 `0` 表示尽量抓全；运行中会自动下拉展开更多回复，并且按 checkpoint 持续保存中间结果。</div>
          <button class="btn" type="submit">开始抓取指定帖子评论</button>
        </form>
  ...
  <script>
    ...
    async function submitTask(form) {
      ...
      const endpointMap = {
        ...
        post_comments: "/api/tasks/post-comments",
...
@app.post("/api/tasks/post-comments")
def api_task_post_comments():
    post_url = (request.form.get("post_url") or "").strip()
    if not post_url:
        return jsonify({"error": "帖子链接不能为空。"}), 400
    try:
        max_comments = int((request.form.get("max_comments") or "0").strip() or "0")
        max_scrolls = int((request.form.get("max_scrolls") or "400").strip() or "400")
        no_new_stop = int((request.form.get("no_new_stop") or "12").strip() or "12")
        scroll_pause = int((request.form.get("scroll_pause") or "1800").strip() or "1800")
    except ValueError:
        return jsonify({"error": "评论上限、滚动轮数、停止轮数和滚动间隔必须是整数。"}), 400
    if max_comments < 0:
        return jsonify({"error": "评论上限不能小于 0。"}), 400
    if max_scrolls <= 0 or no_new_stop <= 0 or scroll_pause <= 0:
        return jsonify({"error": "滚动轮数、停止轮数和滚动间隔必须大于 0。"}), 400
    task_id = start_task(
        "post_comments",
        {
            "post_url": post_url,
